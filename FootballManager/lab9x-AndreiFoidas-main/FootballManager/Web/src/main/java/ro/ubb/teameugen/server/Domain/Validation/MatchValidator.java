...
public class MatchValidator implements BaseValidator<Match> {
    ...
    public void validate(Match match) throws MatchValidationException {
        Optional.of(match.getClubID1())
                .filter(id -> id > 0)
                .orElseThrow(() -> new MatchValidationException("Club ID cannot be negative or zero."));

        Optional.of(match.getClubID2())
                .filter(id -> id > 0)
                .orElseThrow(() -> new MatchValidationException("Club ID cannot be negative or zero."));

        Optional.of(match.getClubID1())
                .filter(id -> id != match.getClubID2())
                .orElseThrow(()->new MatchValidationException("ClubID1 cannot be equal to ClubID2."));

        Optional.of(match.getCompetitionID())
                .filter(id -> id > 0)
                .orElseThrow(() -> new MatchValidationException("Competition ID cannot be negative or zero."));
